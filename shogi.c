...
char board[ROW][COLUMN] = {
    ...
    {'p', 'p', 'p', 'p', 'p', 'p', 'p', 'p', 'p'},
    {' ', 'f', ' ', ' ', ' ', ' ', ' ', 'a', ' '},
    {'c', 'h', 's', 'g', 'k', 'g', 's', 'h', 'c'}}; /*initialize the shogi board*/
    ...
    for (int i = 8; i >= 0; i--)
    {
        ...
        for (int j = 0; j < COLUMN; j++)
        {
            ...
            case ' ':
                ...
                break;
            case 'k':
                printf("\033[31m 王 \033[0m");
                break;
            case 'g':
                printf("\033[31m 金 \033[0m");
                break;
            case 's':
                printf("\033[31m 銀 \033[0m");
                break;
            case 'h':
                printf("\033[31m 桂 \033[0m");
                break;
            case 'c':
                printf("\033[31m 香 \033[0m");
                break;
            case 'a':
                printf("\033[31m 角 \033[0m");
                break;
            case 'f':
                printf("\033[31m 飛 \033[0m");
                break;
            case 'p':
                printf("\033[31m 步 \033[0m");
                break;
            case 'K':
                printf("\033[34m 王 \033[0m");
                break;
            case 'G':
                printf("\033[34m 金 \033[0m");
                break;
            case 'S':
                printf("\033[34m 銀 \033[0m");
                break;
            case 'H':
                printf("\033[34m 桂 \033[0m");
                break;
            case 'C':
                printf("\033[34m 香 \033[0m");
                break;
            case 'A':
                printf("\033[34m 角 \033[0m");
                break;
            case 'F':
                printf("\033[34m 飛 \033[0m");
                break;
            case 'P':
                printf("\033[34m 步 \033[0m");
                break;
            }
        }
        switch (i)
...
int push(int from1, int from2, int to1, int to2)
{
    ...
    if ((tmp = (NODE *)malloc(sizeof(NODE))) != NULL)
    {
        ...
        tmp->to_s[0] = to1;
        tmp->to_s[1] = to2;
        tmp->f_chess = board[from1][from2];
        tmp->t_chess = board[to1][to2];
        tmp->stepcounter = counter;
        tmp->next = NULL;
        tmp->prev = NULL;
        if(cur != NULL){
            cur->prev = tmp;
...
int pop()
{
    ...
    tmp = stack;
    stack = stack->next;
    board[tmp->from_s[0]][tmp->from_s[1]] = tmp->f_chess;
    board[tmp->to_s[0]][tmp->to_s[1]] = tmp->t_chess;
    free(tmp);
    return 0;
}
...
{ // review step
    ...
    board[tmp->to_s[0]][tmp->to_s[1]] = board[tmp->from_s[0]][tmp->from_s[1]];
    board[tmp->from_s[0]][tmp->from_s[1]] = ' ';
    printf("**********%d**********\n", step + 1);
    chessboard();
    backstep = false;
    while (check)
    {
        ...
        else
        {
            ...
            { // gamecheck function to update chessboard
                tmp = tmp->prev;
                if (step +1 == stack->stepcounter)
                {
...
{ // main page in system
    ...
    while (check)
    {
        ...
        else
        {
            ...
            { // reset chessboard and start time loop
                printf("Game starts!\n");
                while (gamereset)
                {
                    pop();
            ...
            { // review game or call binary file
...
{ // shogi rule to check the step is right
    int unit1 = 0, unit2 = 0;
    check = false;
